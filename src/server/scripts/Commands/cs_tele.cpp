...
void createCircle(Player* player) {

    float x = player->GetPositionX();
    float y = player->GetPositionY();   

    float x_high = x + 100.f;
...
void spawnCreatureAroundCircle(Player* player) {
    float x = player->GetPositionX(); 
    float y = player->GetPositionY(); 


    for (int i = -100; i <= 102; i += 1)
    {   
        float y2 = sqrt(1000 - pow(i, 2));
        player->SummonCreature(500500, x + i, y2 + y, player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 100000);
        player->SummonCreature(500500, x + i, -y2 + y, player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 100000);
    }
}
...
public:
    ...
    static bool HandleTeleAddCommand(ChatHandler* handler, std::string const& name)
    {
        ...
        spawnCreatureAroundCircle(player);
