...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "addpoints",          rbac::RBAC_PERM_COMMAND_ADDITEM,          false, &HandleAddPointsSpawnBTR,          "" },
            { "joinbtr",          rbac::RBAC_PERM_COMMAND_ADDITEM,          false, &HandleAddJoinBTR,          "" },
            { "addchest",          rbac::RBAC_PERM_COMMAND_ADDITEM,          false, &HandleAddChestBTR,          "" },
    ...
    static bool HandleAddJoinBTR(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        Battleroyale::addPlayer(playerTarget, 1);

    }

    static bool HandleAddPointsSpawnBTR(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        WorldDatabase.PQuery("INSERT INTO battleroyale_spawns (id, x, y, z, mapId) VALUES (%u, %f, %f, %f, %u)", id, playerTarget->GetPositionX(), playerTarget->GetPositionY(), playerTarget->GetPositionZ(), playerTarget->GetMapId());

    }

    static bool HandleAddChestBTR(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        WorldDatabase.PQuery("INSERT INTO battleroyale_chests_spawns (id, x, y, z, mapId) VALUES(%u, %f, %f, %f, %u)", id, playerTarget->GetPositionX(), playerTarget->GetPositionY(), playerTarget->GetPositionZ(), playerTarget->GetMapId());
    }
