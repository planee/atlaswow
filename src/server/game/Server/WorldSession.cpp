...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player) {
        uint32 gameId = Battleroyale::checkIfPlayerIsInBattleRoyale(_player);
        if (gameId > 0) {
            if (Battleroyale::gameIsStarted(gameId)) {
                if (_player->GetGroup())
                    Battleroyale::removePlayerFromGroup(_player, true);
                else
                    Battleroyale::removePlayer(_player, false);
            }
            else {
                Battleroyale::removePlayer(_player);
            }
        }
    }
