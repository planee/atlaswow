...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recvData)
{
    ...
    if (Battleroyale::checkIfPlayerIsInBattleRoyale(invitingPlayer) > 0) {
        ChatHandler chat(invitingPlayer->GetSession());
        chat.SendSysMessage("You can't invite someone when you are in battle royale queue.");
        return;
    }

    if (Battleroyale::checkIfPlayerIsInBattleRoyale(invitedPlayer) > 0) {
        ChatHandler chat(invitingPlayer->GetSession());
        chat.SendSysMessage("You try to invite someone that in battle royale queue.");
        return;
    }
